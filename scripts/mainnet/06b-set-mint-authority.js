 ...
 *
 * Same as 06-set-mint-authority.js but reads the mint from gold-mint-mainnet-info.json
 * (which now holds the NEW metadata-bearing mint) and transfers authority from the
 * deployer to the GameConfig PDA. Mints nothing — pure authority handoff.
 *
 * Usage: node scripts/mainnet/06b-set-mint-authority.js
 */
...
async function main() {
  if (!fs.existsSync(MINT_INFO)) { console.error("❌ gold-mint-mainnet-info.json not found."); process.exit(1); }
  ...
  if (!cfgAcct) { console.error("❌ GameConfig PDA not found."); process.exit(1); }
  ...
  if (curAuth === gameConfigPda.toBase58()) {
    ...
    if (curAuth !== payer.publicKey.toBase58()) {
      console.error("❌ Mint authority is neither deployer nor GameConfig PDA:", curAuth);
      process.exit(1);
    }
    console.log("\n>>> Transferring authority deployer → GameConfig PDA...");
  ...
  console.log("Supply        :", (Number(fin.supply) / 1e9).toLocaleString(), "GOLD (must be 0)");
  console.log("Authority = GameConfigPDA ?", fin.mintAuthority?.toBase58() === gameConfigPda.toBase58() ? "✅" : "❌");

  const inf = JSON.parse(fs.readFileSync(MINT_INFO, "utf8"));
  inf.mintAuthority = gameConfigPda.toBase58();
  fs.writeFileSync(MINT_INFO, JSON.stringify(inf, null, 2));
  console.log("\nUpdated:", MINT_INFO);
}
