...
interface Spark {
  ...
  w: number;
  h: number;
  dx: number;  // pre-computed per-frame x step
}
...
export function GoldEye({ goldCount }: GoldEyeProps) {
  ...
  // Update ref in-place — prevents useEffect restarts
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (hasGold) {
        ...
        const spawnInterval = Math.max(3, 22 - Math.floor(gc * 0.25));
        const maxSparks = Math.min(80, 12 + Math.floor(gc * 1.5));
        const speedBase = 0.02 + gc / 700;
          ...
          if (sparksRef.current.length < maxSparks) {
            ...
            if (inEllipse(sx, sy, eyeRx, eyeRy)) {
              sparksRef.current.push({
                ...
                w: sparkSize,
                h: sparkSize,
                dx: speedBase * (0.5 + Math.random()),
              });
            }
          }
        }

        // Update & draw sparks — no sine/cosine per frame, just subtract dx
